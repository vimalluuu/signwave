...
async function makeIOS(
  ...
  await Promise.all([
    ...
    promisify(fs.writeFile)(filePath(locale, 'privacy_url.txt'), `https://signwave.com/legal/privacy?lang=${locale}`),
    promisify(fs.writeFile)(filePath(locale, 'marketing_url.txt'), `https://signwave.com/about?lang=${locale}`),
...
async function iOSMetadata() {
  ...
  await Promise.all([
    promisify(fs.writeFile)(`${IOS_META}/copyright.txt`, `${new Date().getFullYear()} signwave.com ltd`),
